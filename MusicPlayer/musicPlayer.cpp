...
private:
    ...
    // Delete song
    void deleteFromCSV(string title, string artist)
    {
        ifstream file(CSV_FILE);
        ...
        while (getline(file, line))
        {
            ...
            string fields[5];
            if (!csv.parseLine(line, fields, 5))
                continue;
    ...
    void runTimer(Song s)
    {
        ...
        cout << "\n  Controls: [P] Pause/Resume  [S] Stop\n\n";
...
public:
    ...
    void deleteSong()
    {
        ...
        do
        {
            if (toLower(temp->song.title) == toLower(title) &&
                toLower(temp->song.artist) == toLower(artist))
            {
                found = true;
